# Vibe Coders League - Platzi 2026

Repositorio con los 13 retos del Vibe Coders League de Platzi 2026.

## Retos

| # | Reto | Estado |
|---|------|--------|
| 01 | [Vibecode Summit Colombia](./reto-01) | Completado |
| 02 | [Music DNA Dashboard](./reto-02) | Completado |
| 03 | [Reto 3](./reto-03) | Pendiente |
| 04 | [Reto 4](./reto-04) | Pendiente |
| 05 | [Reto 5](./reto-05) | Pendiente |
| 06 | [Reto 6](./reto-06) | Pendiente |
| 07 | [Reto 7](./reto-07) | Pendiente |
| 08 | [Reto 8](./reto-08) | Pendiente |
| 09 | [Reto 9](./reto-09) | Pendiente |
| 10 | [Reto 10](./reto-10) | Pendiente |
| 11 | [Reto 11](./reto-11) | Pendiente |
| 12 | [Reto 12](./reto-12) | Pendiente |
| 13 | [Reto 13](./reto-13) | Pendiente |

## Estructura

```
├── reto-01/          # Cada reto es un proyecto independiente
├── reto-02/
├── ...
└── reto-13/
```

Cada carpeta `reto-XX/` contiene su propio proyecto con su README y código.
