# Professor AI Gem — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write the reto-13 README documenting a complete system prompt for a Gemini Gem that acts as an academic AI tutor.

**Architecture:** Single README.md written in Spanish, containing the system prompt (in English), design rationale, prompt anatomy, two example conversations (in English), analysis, and conclusions. Follows the documentation pattern of reto-12.

**Tech Stack:** Markdown only. No code, no dependencies.

**Spec:** `reto-13/docs/superpowers/specs/2026-03-24-professor-ai-gem-design.md`

---

### Task 1: Header and challenge description

**Files:**
- Modify: `reto-13/README.md`

- [ ] **Step 1: Write header and challenge description**

Overwrite the entire contents of `reto-13/README.md` with:
...
Run: `cat reto-13/README.md`
Expected: The header, description, status, and challenge table are present.

- [ ] **Step 3: Commit**

```bash
git add reto-13/README.md
git commit -m "feat(reto-13): add header and challenge description"
```
...
- [ ] **Step 1: Add design rationale section**

Append after the challenge section:
...
```bash
git add reto-13/README.md
git commit -m "feat(reto-13): add design rationale section"
```

---

### Task 3: The complete system prompt
...
Append after the design rationale:
...
```bash
git add reto-13/README.md
git commit -m "feat(reto-13): add complete system prompt"
```

---

### Task 4: Prompt anatomy

**Files:**
- Modify: `reto-13/README.md`

- [ ] **Step 1: Add prompt anatomy section**

Append after the system prompt. **Note:** This section contains code block snippets inside the content. Use the Write tool to append the content directly — do not try to copy-paste from nested backticks.

The section to append is `## 4. Anatomia del Prompt` with the following subsections:

**4.1 Rol definido** — Quote the first two paragraphs of the system prompt ("You are Professor AI..."). Explain: establishes identity, expertise level, and domain. Technique: role assignment. Effect: academic authority, precise terminology, calibrated to intermediate student.

**4.2 Tono** — Quote the TONE AND LANGUAGE section header and first two rules. Explain: defines communication style (language, formality, precision, intellectual honesty). Technique: consistent tone. Effect: uniform register across all responses.

**4.3 Formato de respuesta** — Quote the RESPONSE FORMAT five-section list. Explain: imposes five-section structure on every response. Technique: predictable output format. Effect: scannable, complete, consistent responses.

**4.4 Restricciones** — Quote the RESTRICTIONS section header and first two rules. Explain: sets domain boundaries, uncertainty behavior, prohibited content. Technique: content and behavior restrictions. Effect: prevents domain drift, fabrication, unfounded opinions.

**4.5 Mapeo completo** — A summary table mapping each challenge requirement to the prompt component:
...
```bash
git add reto-13/README.md
git commit -m "feat(reto-13): add prompt anatomy breakdown"
```

---

### Task 5: Example conversations

**Files:**
- Modify: `reto-13/README.md`

- [ ] **Step 1: Add example conversations section**

Append after the anatomy section. The section header in Spanish introduces the conversations, but the actual Q&A content is in English (matching the Gem's language).

**IMPORTANT:** The example responses below should ideally be generated by sending the system prompt to an actual Gemini Gem and capturing real responses. If Gemini access is not available during execution, use the responses below which faithfully follow the five-section structure.

The section to append is `## 5. Conversaciones de ejemplo` with introductory text:
...
**Conversation 1: Explicacion de concepto**

**Pregunta:** What is LoRA and how does it compare to full fine-tuning?

Response (in blockquote format):
...
**Conversation 2: Comparacion de herramientas**
...
```bash
git add reto-13/README.md
git commit -m "feat(reto-13): add example conversations"
```
...
Append after the example conversations:
...
- [ ] **Step 2: Add conclusions section**

Append after the analysis:
...
- [ ] **Step 3: Update status at the top of the README**

Change `**Estado:** En progreso` to `**Estado:** Completado`

- [ ] **Step 4: Commit**

```bash
git add reto-13/README.md
git commit -m "feat(reto-13): add analysis and conclusions"
```
...
- [ ] **Step 1: Update the reto-13 entry in the main README table**
...
```bash
git add README.md
git commit -m "docs(reto-13): mark as completed in main README"
```
